...
kubectl create namespace gatekeeper-system   --dry-run=client   -o yaml |
kubectl apply -f -
...
helm upgrade --install gatekeeper gatekeeper/gatekeeper   --version "$GATEKEEPER_VERSION"   --namespace gatekeeper-system   --wait   --timeout 10m
...
wait_for_crd() {
  ...
  until kubectl get "crd/${crd_name}" >/dev/null 2>&1; do
    if (( elapsed >= timeout_seconds )); then
      ...
      kubectl get constrainttemplates -o yaml >&2 || true
      exit 1
    fi
...
wait_for_crd   "k8srequirenonroots.constraints.gatekeeper.sh"

wait_for_crd   "k8srequiresignatureannotations.constraints.gatekeeper.sh"
...
helm upgrade --install   aws-load-balancer-controller   eks/aws-load-balancer-controller   --version "$AWS_LBC_CHART_VERSION"   --namespace kube-system   --set "clusterName=${EKS_CLUSTER_NAME}"   --set "region=${AWS_REGION}"   --set "vpcId=${VPC_ID}"   --set "serviceAccount.create=false"   --set "serviceAccount.name=aws-load-balancer-controller"   --wait   --timeout 10m

kubectl create namespace "$MONGODB_APP_NAMESPACE"   --dry-run=client   -o yaml |
kubectl apply -f -

MONGODB_URL="$(terraform output -raw mongodb_app_connection_string)"
SECRET_KEY="$(terraform output -raw jwt_secret)"

kubectl create secret generic "$MONGODB_APP_SECRET_NAME"   --namespace "$MONGODB_APP_NAMESPACE"   --from-literal="MONGODB_URL=${MONGODB_URL}"   --from-literal="SECRET_KEY=${SECRET_KEY}"   --dry-run=client   -o yaml |
kubectl apply -f -

kubectl get namespace   gatekeeper-system   "$MONGODB_APP_NAMESPACE"

kubectl get serviceaccount   aws-load-balancer-controller   --namespace kube-system   -o jsonpath='{.metadata.annotations.eks\.amazonaws\.com/role-arn}{"\n"}'
